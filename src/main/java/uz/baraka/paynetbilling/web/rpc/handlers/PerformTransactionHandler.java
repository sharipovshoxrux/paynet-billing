...
class PerformTransactionHandler implements RpcHandler {
    ...
    public JsonRpcModels.Response handle(Object id, Map<String,Object> params) {
        ...
        var info = billing.perform(p.transactionId(), p.fields().application_id(), p.amount());
        ...
        return JsonRpcModels.Response.ok(
                id,
                new JsonRpcModels.Result(null, JsonRpcController.now(clock), fields)
        );
    }
}
